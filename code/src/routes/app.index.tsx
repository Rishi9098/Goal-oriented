...
export const Route = createFileRoute("/app/")({
  ...
  staticData: { shellTitle: "Overview" },
  component: Dashboard,
});
...
// Milestone 2 Task 12 — one quiet card surfacing the Family dashboard on
// the main overview. Deliberately renders nothing while loading or on
// error: the money dashboard must never break or grow a second spinner
// because the family aggregate is unavailable (UserTrustAndDesignReview_
// Task12.md).
function FamilyCard() {
  const { data } = useQuery({
  ...
  if (!data?.dependents) return null;

  const people = data.dependents.total_members;
  const suggestionCount = data.recommendations.length;
  const suggestionText =
    suggestionCount > 0
      ? `${suggestionCount} suggestion${suggestionCount === 1 ? "" : "s"} to review`
      : "No open suggestions";
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <Users className="h-5 w-5 text-cyan shrink-0" aria-hidden="true" />
        <div>
          <p className="text-sm font-medium">
            Family — {people === 1 ? "just you right now" : `${people} people`}
          </p>
          <p className="text-xs text-muted-foreground">{suggestionText}</p>
        </div>
      </div>
...
function Dashboard() {
  ...
  return (
    ...
    >
        ...
        {loading ? (
          ...
          <>
            <Stat
              ...
              delta={
                dashData?.net_worth_delta_pct ? `+${dashData.net_worth_delta_pct}% YTD` : undefined
              }
              positive
            />
            ...
            <Stat
              ...
              delta={
                dashData?.monthly_savings_rate
                  ? `${dashData.monthly_savings_rate}% of income`
                  : undefined
              }
              positive={true}
            />
      ...
      {/* Family summary (Milestone 2 Task 12) */}
      <FamilyCard />

      {/* Recent life events (LifeEventIntegrationReview.md Phase 2) — the
          Dashboard is the first screen a user sees, so this is a "prompt"
          empty state (record your first one) rather than hidden entirely. */}
      <RecentLifeEventsCard emptyState="prompt" />
          ...
          <div className="grid grid-cols-3 divide-x divide-border">
            <div className="pr-4">
              ...
              <p className="mt-1 font-display text-xl text-success">
                {formatCurrency(dashData.monthly_income)}
              </p>
            </div>
            <div className="px-4">
              <p className="text-xs uppercase tracking-widest text-muted-foreground">
              ...
              <p className="mt-1 font-display text-xl text-warning">
                {formatCurrency(dashData.monthly_expenses)}
              </p>
            </div>
            <div className="pl-4">
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <div className="surface-card p-5">
          ...
          {/* Always true, not conditional on any specific event — recommendations
              are computed live on every read (ADR-005), so this never overclaims
              a causal link the system can't verify (LifeEventIntegrationReview.md
              Phase 2 §4/§5). */}
          <p className="mt-3 text-[11px] text-muted-foreground">
            Recommendations always reflect your latest numbers, including anything you've recorded
            as a life event.
          </p>
          <Link
            to="/app/copilot"
            className="mt-3 w-full inline-flex items-center justify-center gap-1.5 rounded-lg border border-border-strong bg-surface px-3 py-2 text-sm hover:bg-accent transition"
          >
